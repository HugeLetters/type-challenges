...
type TypeRecord = Primitive & NonPrimitive;

type BaseSchema = { type: keyof TypeRecord | (keyof TypeRecord)[] };
type TypeFromJSONSchema<T extends BaseSchema> = TypeRecord[T['type'] extends (keyof TypeRecord)[]
   ? T['type'][number]
   : T['type'] extends keyof TypeRecord
   ? T['type']
   : never];
type EnumJSONSchema<T> = { enum: T[] };
type ObjectJSONSchema = { properties: Record<string, BaseSchema>; required?: string[] };
type ArrayJSONSchema = { items: BaseSchema | BaseSchema[] };

type JSONSchema2TS<
   T extends BaseSchema,
   Type = TypeFromJSONSchema<T>
> = T extends EnumJSONSchema<Type>
   ? T['enum'][number]
      ...
      ? Omit<
           {
              ...
              ? {
                   [K in T['required'][number] & keyof T['properties']]: JSONSchema2TS<
                      T['properties'][K]
                   >;
                }
              : unknown),
   ...
   ? T extends ArrayJSONSchema
      ? T['items'] extends (infer I)[]
         ? (I extends BaseSchema ? JSONSchema2TS<I> : never)[]
         : T['items'] extends BaseSchema
