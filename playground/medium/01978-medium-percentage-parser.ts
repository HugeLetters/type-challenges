...
type SignPart<A extends string> = A extends `${"+" | "-"}${infer Right}`
  ? A extends `${infer R}${Right}`
    ? R
    : never
  : ""
type PercentagePart<A extends string> = A extends `${infer Left}${"%"}`
  ? A extends `${Left}${infer R}`
    ? R
    : never
  : ""
type PercentageParser<A extends string> =
  A extends `${SignPart<A>}${infer Digits}${PercentagePart<A>}`
    ? [SignPart<A>, Digits, PercentagePart<A>]
    : ""
