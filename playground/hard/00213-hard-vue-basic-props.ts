...
type ParseConstructor<C extends Class, V = InstanceType<C>> = V extends { valueOf(): infer R }
   ? R extends Record<string, any>
      ? V
      : R
   : V;
type ParsePropsRecord<T> = T extends Record<
   string,
   Record<PropertyKey, never> | Class | { type: Class | Class[] }
>
   ? {
        [K in keyof T]: T[K] extends { type: infer A }
           ? A extends Class[]
              ? ParseConstructor<A[number]>
              : A extends Class
              ? ParseConstructor<A>
              : any
           : T[K] extends Class
           ? ParseConstructor<T[K]>
           : any;
     }
   : unknown;

declare function VueBasicProps<D, C, M, P>(
   options: {
      data: (this: ParsePropsRecord<P>) => D;
      computed: C;
      methods: M;
      props: P;
   } & ThisType<D & ParseComputedRecord<C> & ParseMethodRecord<M> & ParsePropsRecord<P>>
): typeof options;
