...
type ParseGetters<G> = G extends Record<PropertyKey, () => any>
   ? { [K in keyof G]: ReturnType<G[K]> }
   : never;

declare function defineStore<S, G, A, V = ParseGetters<G>>(store: {
   id: string;
   state: () => S;
   getters: G & ThisType<Readonly<S> & V>;
   actions: A & ThisType<S & Readonly<V> & A>;
}): S & V & A;
